...
The commit is merge-shaped, with the previous `fork_upstream` tip as its first parent and the upstream tip as its second. That is an implementation detail, but it is what preserves upstream history. `git log`, `git blame`, contributor attribution, the changelog, and the meta commit (ADR-023) all work unchanged. Every generated commit records `Upstream-Sha` and `Filter-Rev` trailers, where `Filter-Rev` identifies both the filter engine version and the effective per-service configuration, so any generated tree can be reproduced exactly from its inputs.
